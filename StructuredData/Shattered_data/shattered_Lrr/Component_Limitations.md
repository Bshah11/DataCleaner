...
## Specific rule - [If a component type is depleted during the game, players obey the following rules:]
### Rule points
1. Dice: Dice are limitless. If a player needs to roll more dice than the game provides, that player should roll as many as possible, record the results, and then reroll dice as necessary.
2. Tokens: Tokens are limited to those included in the game, except for the following:
3. If any of the above tokens are depleted, players can use a suitable substitute, such as a coin or bead.
4. Units: Units are limited to those included in the game, except for fighters and ground forces.
...
5. Cards: When a deck is depleted, players shuffle the deck’s discard pile and place it facedown to create a new deck.
